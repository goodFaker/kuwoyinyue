	...
	<div class="outer">
		...
		<div class="container">
			...
			<div class="info_r">
				<div>
					<p class="songName">
						<em>{{ musicObj.artist }}</em>
						<i class="iconfont icon-mv"></i>
					</p>
					<p class="name">{{ musicObj.artist }}</p>
					<p class="albumName">
					...
					<div class="but">
						...
						>
							立即播放</el-button
						>
						<el-button round class="iconfont icon-icon-test">
							添加</el-button
						>
						<el-button round class="iconfont icon-shoucang">
							收藏</el-button
						>
						<el-button round class="iconfont icon-share">
							分享</el-button
						>
						<el-button round class="iconfont icon-pinglun">
							评论</el-button
						>
					</div>
					<!-- 歌词区域 -->
...
export default {
	...
	async mounted() {
		/* 请求音频地址 */
		const musicObj = JSON.parse(sessionStorage.getItem('music'))
    this.musicObj = musicObj
		let  rid  = musicObj.rid
		let musicUrl = await this.$API.reqPlayInfo(rid)
